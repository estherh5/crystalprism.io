...
[Interactive](http://crystalprism.io/interactive/index.html) incorporates SVGs and randomization to generate a peaceful scene where the user can clear shapes as they clear their mind. I use JavaScript to generate a random shape from a set of 10 SVGs and give it a set of random coordinates. When the user clicks the outline of one of the shapes, the shape gets "blasted" out of the scene, clearing the space for the heart rain that continues to fall in the background, generated from SVG cloning. The rain and shapes will clear on their own after a certain time interval to minimize system performance.
