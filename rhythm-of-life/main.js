...
var scale = 60; // Length/width of one unit of game board (used for sizing and moving relievers/stressors)
...
for (var i = 0; i < document.getElementsByClassName('reliever').length; i++) {
  document.getElementsByClassName('reliever')[i].addEventListener('load', function() {
...
for (var i = 0; i < document.getElementsByClassName('stressor').length; i++) {
  document.getElementsByClassName('stressor')[i].addEventListener('load', function() {
...
document.getElementById('heart').onload = function() {
  heart = Snap(document.getElementById('heart').contentDocument.getElementsByTagName('svg')[0])
  .attr({fill: '#56d056', 'x': 0, 'y': 0, width: scale, height: scale});
  svgGame.append(heart);
}

// Set stylesheet to mobile version on load if user is on mobile device
function setMobileStyle() {
  // Assess if user is on mobile device and set mobile variable, scale, and stylesheet accordingly
  (function(a) {
...
function place(objs) {
  ...
  // Clone random object and place at random x- and y-coordinates on game board based on scale size
...
// Assess that game is ready to start if Start screen is the only screen displayed
function readyToStart() {
  if ((startScreenMobile.style.visibility == 'visible' || startScreenDesktop.style
  .visibility == 'visible') && (learnScreen.style.visibility != 'visible' && leaderScreen
...
// Assess that game is paused if Pause screen, Learn More screen, or Leaders screen is visible
function paused() {
  if (pauseScreenDesktop.style.visibility == 'visible' || pauseScreenMobile.style
  .visibility == 'visible' || learnScreen.style.visibility == 'visible' || leaderScreen
  .style.visibility == 'visible') {
    return true;
  }
...
// Determine initial x- and y-coordinates where user touched the screen when swiping on mobile
...
// Determine final x- and y-coordinates where user touched the screen when swiping on mobile
document.getElementById('game-board').addEventListener('touchmove', function(e) {
  ...
  // Determine final x- and y-coordinates where user touched the screen and the difference between the start and end values
  ...
  if (Math.abs(xDiff) > Math.abs(yDiff)) {
    // If starting x-coordinate is greater than ending x-coordinate, user swiped left
    if (xDiff > 0) {
      direction = 'ArrowLeft';
    }
  ...
  // If user swiped up/down, set direction as appropriate
    ...
    // If starting y-coordinate is greater than ending y-coordinate, user swiped up
    if (yDiff > 0) {
    direction = 'ArrowUp';
    }
...
document.body.onkeydown = function(e) {
  ...
  else if (e.key == 'ArrowUp' || e.key == 'ArrowDown' || e.key == 'ArrowRight' || e
  .key == 'ArrowLeft') {
    e.preventDefault();
    direction = e.key;
    // Start/resume game if it hasn't started yet or is paused
...
function gameLoop() {
  ...
  // Assess if stressor and reliever collided and replace reliever with another if so
...
function updateLifespan() {
  ...
  lifespan.innerHTML = 'Lifespan: ' + ('0' + hours).slice(-2) + ':' + ('0' + minutes)
...
function move(obj, direction) {
  ...
  // If object is now off game board (x is too far right), set x to far-left of game board
  if (obj.attr('x') >= gameWidth) {
    obj.attr({'x': 0});
    return;
  }
  // If object is now off game board (x is too far left), set x to far-right of game board
  if (obj.attr('x') < 0) {
    obj.attr({'x': gameWidth});
    return;
  }
  // If object is now off game board (y is too far down), set y to top of game board
  if (obj.attr('y') >= gameLength) {
    obj.attr({'y': 0});
    return;
  }
  // If object is now off game board (y is too far up), set y to bottom of game board
  if (obj.attr('y') < 0) {
...
// Assess if object A collided with object B, and perform specified function if so
function didCollide(objA, objB, action) {
  // If object A and object B are at the same x- and y-coordinates on the game board, return true and perform specified function
...
// If stressor collided with reliever, remove reliever from and place a new reliever on game board
...
function setHeartAttr() {
  ...
  systolicText.innerHTML = systolic;
  diastolicText.innerHTML = diastolic;
  // If systolic and diastolic values are in normal range, set heart and BP to green color and normal heartbeat speed
  ...
  // If systolic and diastolic values are in prehypertension range, set heart and BP to yellow color and increase heartbeat speed
  ...
  // If systolic and diastolic values are in hypertension stage 1 range, set heart and BP to orange color and increase heartbeat speed
  ...
  // If systolic and diastolic values are in hypertension stage 2 range, set heart and BP to red color and increase heartbeat speed
...
function storeScore() {
  ...
  var entry = {
    ...
    lifespan: ('0' + hours).slice(-2) + ':' + ('0' + minutes).slice(-2) + ':' + ('0' + seconds)
    .slice(-2),
  };
  ...
  return fetch(server + '/rhythm-of-life').then(function(response) {
    response.json().then(function(leadersList) {
      for (var i = 0; i < leadersList.length; i++) {
        ...
        // Clear current leader display on game board and replace with new player and score cells for mobile/desktop
        if (mobile) {
          leadersMobile[i].innerHTML = '';
          leadersMobile[i].append(lifespanCell);
          leadersMobile[i].append(playerCell);
        } else {
          leaders[i].innerHTML = '';
          leaders[i].append(lifespanCell);
          leaders[i].append(playerCell);
        }
        // Display lifespan in lifespan cell
        lifespanCell.append(document.createTextNode(leadersList[i].lifespan));
        // Display link to player's user profile in player cell
        var userLink = document.createElement('a');
        userLink.href = '../user/index.html?username=' + leadersList[i].player;
        userLink.append(document.createTextNode(leadersList[i].player));
        playerCell.append(userLink);
      }
    });
  });
}
...
// Toggle Learn More screen when Learn More or close button is clicked on mobile/desktop
...
function toggleLearnScreen() {
  // Hide Pause and Leaders screens in case they were displayed so only one screen is showing at a time for mobile/desktop
  if (mobile) {
    pauseScreenMobile.style.visibility = 'hidden';
    leaderScreen.style.visibility = 'hidden';
  } else {
  ...
  // Hide Learn More screen if game hasn't started yet or is over and Learn More screen is currently visible
  if ((startScreenDesktop.style.visibility == 'visible' || startScreenMobile.style
  .visibility == 'visible' || gameOver()) && learnScreen.style.visibility == 'visible') {
    learnScreen.style.visibility = 'hidden';
    return;
  }
  ...
  // If game is paused and Learn More screen is currently visible, hide it and resume the game
...
function toggleLeaderScreen() {
  // Hide Pause and Learn More screens in case they were displayed so only one screen is showing at a time
  pauseScreenMobile.style.visibility = 'hidden';
  learnScreen.style.visibility = 'hidden';
  // Hide Leaders screen if game hasn't started yet or is over and Leaders screen is currently visible
  if ((startScreenMobile.style.visibility == 'visible' || gameOver()) && leaderScreen
  .style.visibility == 'visible') {
    leaderScreen.style.visibility = 'hidden';
    return;
  }
  ...
  // If game is paused and Leaders screen is currently visible, hide it and resume the game
...
function toggleSound() {
  ...
  if (heartbeat._muted) {
    ...
    document.getElementById('sound').style.backgroundImage = 'url("images/on.svg")';
    return;
  }
  // Otherwise, mute sound and display Off icon
  heartbeat.mute(true);
  document.getElementById('sound').style.backgroundImage = 'url("images/off.svg")';
  return;
}
