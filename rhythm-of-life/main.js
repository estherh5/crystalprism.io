...
// Define events
window.addEventListener('load', mobileStyle, false);
window.addEventListener('load', checkAccountStatus, false);
window.addEventListener('load', getLeaders, false);

document.body.onkeydown = function(e) {
  // Resume game when space key is pressed
  if (e.key == ' ') {
    e.preventDefault();
    ...
    response.json().then(function(leadersList) {
      for (var i = 0; i < leadersList.length; i++) {
        ...
        if (leadersList[i].status == 'user') {
          ...
          tableContainerTwo.append(document.createTextNode(leadersList[i].player));
        }
      }
      for (var i = 0; i < leadersList.length; i++) {
        var tableContainerOne = document.createElement('td');
